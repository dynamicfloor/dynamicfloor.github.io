...
//             if (indicador) {
//                 indicador.innerText = estado ? " *" : "(Optativo)";
...
function aplicarEstructuraConfig() {
    Object.entries(FORM_STRUCTURE_CONFIG).forEach(([key, estado]) => {
        const card = document.querySelector(`.form-card[data-campo="${key}"]`);
        if (!card) return;
        ...
        if (estado === null) {
            ...
            const indicador = card.querySelector(".indicador-estado");
            if (indicador) {
                // Usamos innerHTML e inyectamos un span válido
                indicador.innerHTML = estado 
                    ? ' <span class="requerido">*</span>' 
                    : ' <span class="opcional">(opcional)</span>';
                
                indicador.style.color = estado ? "var(--pink, #ff4a5a)" : "inherit";
            }
        }
    });
}
